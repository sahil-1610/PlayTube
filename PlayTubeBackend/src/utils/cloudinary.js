  ...
  const uploadVideoOnCloudinary = async (localFilePath) => {
    try {
      ...
      const cldnry_res = await cloudinary.uploader.upload(localFilePath, {
        ...
        eager: [
          {
            streaming_profile: "sd", // Streaming profile, can be 'full_hd' or 'sd' etc.
     ...
     console.log("yaha tak done , cloundinary upload")
  
      // Checking if eager transformation returned the HLS URL
      const hlsurl = cldnry_res.eager && cldnry_res.eager[0] ? cldnry_res.eager[0].secure_url : undefined;
